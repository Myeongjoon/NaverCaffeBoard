			...
			<div class="content">
				...
				<fieldset>
					<form modelAttribute="LoginProcess" id="thisform" name="LoginForm"
						action="/pro/spring_security_check" method="post">
						<input type="hidden" id="serviceId" name="serviceId"
							value="CONNECT" /> <input type="hidden" id="targetUrl"
							name="targetUrl" uniInfo="nssfhrmdlsvpdlwl"
							value="http://connect.navercorp.com/" /> <input type="hidden"
							id="etnlLoginYn" name="etnlLoginYn" value="N" /> <input
							type="hidden" id="loginLocation" name="loginLocation"
							value="nssLogin" /> <input type="hidden" id="changeLT"
							name="changeLT" value="false" />
						<legend class="blind">Login Information</legend>
						<p class="error_noti" style="display: block"></p>
						<div class="inp_area">
							<div class="inp_txt">
								<input type="text" placeholder="ID" id="j_username" name="j_username"
							...
							<div class="inp_txt">
								<input type="password" placeholder="PASSWORD" id="j_password"
									name="j_password" onkeypress="return executeCustomEnter(event);"
	...
	function customsubmitLogin() {
		...
		if (location.hostname == "localhost") {
			frm.setAttribute("action",
					"http://localhost:8080/pro/spring_security_check");
		} else {
			frm.setAttribute("action",
					"http://10.110.242.87/pro/spring_security_check");
		}

		var selectBox = document.getElementById("password");
		var selectedValue = selectBox.value;
		var inputElement = document.createElement("input");
		inputElement.setAttribute("type", "hidden");
		inputElement.setAttribute("name", "j_password");
		...
		var selectBox = document.getElementById("j_username");
		var selectedValue = selectBox.value;
		var inputElement = document.createElement("input");
		inputElement.setAttribute("type", "hidden");
		inputElement.setAttribute("name", "j_username");
		var encodedValue = encodeURIComponent(selectedValue);
		inputElement.setAttribute("value", encodedValue);
		frm.appendChild(inputElement);
