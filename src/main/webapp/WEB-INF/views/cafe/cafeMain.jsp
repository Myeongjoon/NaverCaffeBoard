            ...
            <c:forEach items="${list }" var="row">
			   <tr align="center">
			      ...
			      <td align="left" class="board-list">
			        <span>
			          <span class="aaa">
			          <a href="/board/cafeMainDetail?boardNo=${row.boardNo}" onclick="clickcr(this, 'cfa.atitle','','',event); return dynamicParamLink('/ArticleRead.nhn?clubid=25158488&amp;page=1&amp;boardtype=L&articleid=929693&referrerAllArticles=true', event);" onmouseover="" class="m-tcol-c">${row.title }</a>
  ...
  <tr>	
  	  ...
  	  <c:choose>
  	  	<c:when test="${currentPage eq page }">
  	  		<a href="/board/cafeMain?page=${page}" onclick="return dynamicParamLink('/board/cafeMain?page=${page}', event)" class="m-tcol-p">${page}</a></td> 
  	  	</c:when>
  	  	<c:otherwise>
  	  		 <td><a href="/board/cafeMain?page=${page}" onclick="return dynamicParamLink('/board/cafeMain?page=${page}', event)" class="m-tcol-c">${page}</a></td>
  	  	</c:otherwise>
  	  	</c:choose> 
  	  	<td class="on">
  	  </c:forEach>
      <!--
      --><td class="pgR"><a href="/board/cafeMain?search.clubid=25158488&search.boardtype=L&search.questionTab=A&search.totalCount=151&search.page=11" onclick="return dynamicParamLink('/board/cafeMain?search.clubid=25158488&search.boardtype=L&search.questionTab=A&search.totalCount=151&search.page=11', event)" class="m-tcol-c pn"><span>다음</span><span class="ico-bl ico">▶</span></a></td> 
  </tr>
</table>      
</div>
